...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, use_flash_attention: bool = False):
        super().__init__()
        self.input_resolution = input_resolution
        ...
        # self.dim_proj = nn.Linear(width, output_dim)
...
def ViT(image_resolution, vision_layers, vision_width, vision_patch_size, use_flash_attention=False, frozen=False, pretrain=None):
    ...
    if pretrain is not None and pretrain != '':
        pretrain_model = torch.load(pretrain)
        vit.load_state_dict(pretrain_model, strict=False)
        print('==========>>>>>>>>>> vit pretrain load successfully')
...
if __name__ == '__main__':
    ...
    config_path = 'config/train_notepair.yaml'
    ...
    pretrain_path = 'clip_cn_vit-b-16-visual.pt'
    model = ViT(**args.model.vision_model.backbone.args)

...
def worker(config_path, world_size, global_rank, device, tag_name):
    ...
    dataset = REDRecEvalItemDataset(global_rank, world_size, config, lastn_note=True)
    ...
    for sample in tqdm(dataloader):
        ...
        for idx in range(len(note_ids)):
            ...
            raw_feature_msg = {
                            'id': note_id, 
                            'embed': cur_raw_feature,  
                            'embed_type': 'cur_embed_64d'
                        }
            
            cnt += 1
            # print(note_id, cur_embed_64d)
            
            embeddings_to_save.append(raw_feature_msg)

    save_local(embeddings_to_save, f"eval/emb/{tag_name}/lastn_item_embedding_pr.pkl")
